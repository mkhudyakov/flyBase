...
func _ready() -> void:
	# "Continue" is only meaningful once a lab save exists.
	_continue_button.disabled = not SaveLoadService.has_save(Lab.SAVE_SLOT)
	_status_label.text = ""
...
func _on_continue_pressed() -> void:
	# Load any saved lab / campaign / economy progress before entering the lab.
	Lab.load_lab()
	Campaign.load_progress()
	Economy.load_progress()
	get_tree().change_scene_to_file(LAB_DASHBOARD_SCENE)
